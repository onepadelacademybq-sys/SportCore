...
export function TechnicalModule({ value, onChange }: Props) {
  function toggle(stroke: string, idx: number) {
    const current = value[stroke] ?? [false, false, false, false, false]
    const next = [...current] as boolean[]
    next[idx] = !next[idx]
    onChange({ ...value, [stroke]: next })
  }
    ...
    <div className="space-y-8">
      {STROKE_GROUPS.map(({ group, label, strokes }) => {
        const totalHits = strokes.reduce(
          (sum, s) => sum + (value[s] ?? []).filter(Boolean).length, 0,
        )
        ...
        return (
          <div key={group}>
            ...
            <div className="rounded-lg border border-border overflow-x-auto">
              ...
              {strokes.map((stroke) => {
                const attempts = value[stroke] ?? [false, false, false, false, false]
