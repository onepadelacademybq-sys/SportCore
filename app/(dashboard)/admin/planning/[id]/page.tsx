...
export default async function AdminMesocycleDetailPage({ params, searchParams }: PageProps) {
    ...
    <div className="p-8 max-w-4xl space-y-6">
      ...
      <div className="flex gap-1 border-b border-border">
        {([
          ...
          <Link
            key={t.key}
            href={`/admin/planning/${id}${t.key ? `?tab=${t.key}` : ''}`}
            ...
            <details key={mc.id} className="group rounded-xl border border-border bg-card" open>
              ...
              <div className="border-t border-border px-5 py-4 space-y-3">
                ...
                {mc.sessions.map((s) => (
                  <Link
                    key={s.id}
                    href={`/admin/planning/${id}/session/${s.id}`}
