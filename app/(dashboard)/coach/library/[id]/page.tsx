...
export const metadata: Metadata = { title: 'Detalle del Ejercicio — Entrenador' }
...
export default async function CoachExerciseDetailPage({ params, searchParams }: PageProps) {
      ...
      {canEdit && (
        <div className="flex gap-1 border-b border-border">
          {(['detail', 'edit'] as const).map((t) => (
            <Link
              key={t}
              href={`/coach/library/${id}${t === 'edit' ? '?tab=edit' : ''}`}
