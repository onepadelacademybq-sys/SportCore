...
export const metadata: Metadata = { title: 'Biblioteca de Ejercicios — Entrenador' }
  ...
  if (exercises.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <Link href="/coach/library/new" className="mt-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {exercises.map((ex) => (
        <div key={ex.id} className="relative group rounded-xl border border-border bg-card hover:border-primary/40 transition-colors flex flex-col">
          ...
          <Link href={`/coach/library/${ex.id}`} className="flex flex-col flex-1 p-4 pt-10">
...
export default async function CoachLibraryPage({ searchParams }: PageProps) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Explora, guarda favoritos y crea tus propios ejercicios
          </p>
        </div>
        <Link href="/coach/library/new">
